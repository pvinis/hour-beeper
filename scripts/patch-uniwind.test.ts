...
import { patchUniwind, REGISTRY_FILES, SAFE_AREA_FILES } from "./uniwind-compat.mjs"
...
describe("patchUniwind", () => {
	it("patches registry and safe-area files and stays idempotent", () => {
		const rootDir = createFixtureRoot()
		const firstRun = patchUniwind({ cwd: rootDir })

		expect(firstRun.patchedFiles).toEqual([...REGISTRY_FILES, ...SAFE_AREA_FILES])
		expect(
			readFileSync(join(rootDir, "node_modules/uniwind/src/components/index.ts"), "utf8"),
		).not.toContain("require('react-native').Clipboard")
		expect(
			readFileSync(
				join(rootDir, "node_modules/uniwind/src/components/native/SafeAreaView.tsx"),
				"utf8",
			),
		).toContain("react-native-safe-area-context")

		const secondRun = patchUniwind({ cwd: rootDir })
		expect(secondRun.patchedFiles).toEqual([])
		expect(secondRun.alreadyPatchedFiles).toEqual([...REGISTRY_FILES, ...SAFE_AREA_FILES])
	})

	it("fails loudly when upstream markers drift", () => {
		const rootDir = createFixtureRoot({
			mutate(filePath, source) {
				if (filePath.endsWith("src/components/index.ts")) {
					return source.replace("Clipboard", "ClipboardCompat")
				}
		...
		expect(() => patchUniwind({ cwd: rootDir })).toThrow(/Uniwind patch drift detected/)
	})
})

function createFixtureRoot(options: {
	mutate?: (filePath: string, source: string) => string
} = {}) {
	const rootDir = mkdtempSync(join(tmpdir(), "hour-beeper-uniwind-compat-"))
	cleanupDirs.push(rootDir)

	for (const filePath of [...REGISTRY_FILES, ...SAFE_AREA_FILES]) {
		const absolutePath = join(rootDir, filePath)
		mkdirSync(dirname(absolutePath), { recursive: true })
		const source = getFixtureSource(filePath)
		writeFileSync(
			absolutePath,
			options.mutate ? options.mutate(filePath, source) : source,
		)
	}
